...
def main(predefined_ontology_path=[]):
    ...
    if not(predefined_ontology_path):
        ontology_opts = ['scenes', 'predefined', 'vg_attributes', 'ovad_attributes']
    else:
    ...
    for vlm_type in ['blip2', 'blip_itc', 'xvlm', 'clip']:
        if vlm_type == 'xvlm':
                ...
                if vlm_type == 'clip' or vlm_type == 'blip_itc':
                    ...
                    if ontology  == 'predefined':
                        if any (df_all_attr_per_obj[df_all_attr_per_obj['object_name'] == visual_objects.names[0]]):
                            if not(df_all_attr_per_obj[df_all_attr_per_obj['object_name'] == visual_objects.names[0]].empty):
